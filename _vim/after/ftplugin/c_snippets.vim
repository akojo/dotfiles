...
function! CMacroName(string)
    return substitute(substitute(a:string,'\.','_','g'),'.*','\U&', '')
endfunction
...
exec "Snippet ginc #include <".st.et."><CR>".st.et
exec "Snippet once #ifndef ``CMacroName(expand('%:t'))``_DEFINED<CR>#define ``CMacroName(expand('%:t'))``_DEFINED<CR><CR>".st.et."<CR><CR>#endif /* ``CMacroName(expand('%:t'))``_DEFINED */<CR>"
